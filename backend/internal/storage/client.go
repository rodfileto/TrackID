...
// the same free-form storageRef shape internal/graph's InfoBio file linking already expects.
...
// InfoBioNISTObjectKey builds the object key this package uses to store a NIF's NIST file.
func InfoBioNISTObjectKey(nif string) string {
	return "infobio/nist/" + url.PathEscape(nif) + ".nist"
}
