...
DTID's knowledge graph is built from four **instance-layer** classes, plus a formal **type layer** grounded in the BFO upper ontology and the Common Core Ontologies (CCO) mid-level ontology (see [`ONTOLOGY_BFO.md`](ONTOLOGY_BFO.md)).
...
### Type layer: BFO + CCO (schema layer)

The types of Target Entities and Target Systems are no longer a hand-rolled, ad-hoc registry (`EntityTypeDefinition` / `TargetTypeTemplate` / `Concept`). They are grounded in a formal upper/mid ontology, imported into Memgraph at startup:

- **BFO 2.0** (ISO/IEC 21838) supplies the top-level categories — continuant vs. occurrent, material entity, immaterial entity, information content entity, process — and the relation set (mereology, participation, temporal precedence).
- **Common Core Ontologies (CCO)** supplies the mid-level classes (Person, Organization, Event, Location, Artifact, Identifier, …), extending BFO + the Relation Ontology.

The full stack — BFO (upper) + RO (relations) + CCO (mid) + a thin `tid:` domain extension — is specified in [`docs/ONTOLOGY_BFO.md`](ONTOLOGY_BFO.md). The upper + mid layers are imported and persisted; the `tid:` domain layer (TrackID-specific types and the per-type confidence-gate defaults that used to live on `EntityTypeDefinition`) is deferred. Until it lands, `Target.target_type` and `TargetEntity.entity_type` are free strings rather than validated against a registry.
...
- Ontology node types: `OntologyClass` (one per BFO/CCO class, keyed by `iri`, with `name`/`definition`/`source`) and `ObjectProperty` (one per relation, with a `transitive` flag) — the taxonomy lives in the same graph, so schema queries ("what classes exist", "is X a subclass of Y") and investigative queries ("what happened in this Target") stay cleanly separable without a cross-store lookup
...
- Ontology edge types: `SUBCLASS_OF` (`OntologyClass`→`OntologyClass`), `DOMAIN`/`RANGE` (`ObjectProperty`→`OntologyClass`)
...
**Threshold tuning**: τ_high and τ_low are policy parameters, not fixed constants — they can differ per Target System, per Target Entity type, or per investigative context, and every threshold change is itself an `AnalystAction`. Per-type defaults will live as `default_tau_high`/`default_tau_low` properties on the relevant `tid:` domain class (deferred — see [`ONTOLOGY_BFO.md`](ONTOLOGY_BFO.md)); until then the platform runs on a single global pair (`RESOLUTION_TAU_HIGH`/`RESOLUTION_TAU_LOW` in `app/core/config.py`) — e.g. `TargetPerson` and `Vehicle` are intended to ship with different default thresholds, reflecting that face-based resolution and plate-based resolution carry different baseline confidence characteristics — rather than floating as an unplaced policy concept.
